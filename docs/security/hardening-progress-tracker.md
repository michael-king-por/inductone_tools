...
| Quality workspace owner decision | Open | Candidate audit confirms `Workspace` `Quality` is public but restricted to retired `Builder`; links include Quality Goal, Quality Procedure, Quality Feedback, Quality Meeting, Non Conformance, Quality Review, and Quality Action. Recommended internal roles for owner approval: Operations Manager, Operations Viewer, optionally Engineering User. No change made. |
...
|---|---|---|
| Custom DocPerm fixture parses | Pass | Local parse validation; 259 rows |
| Custom DocPerm unique keys | Pass | 259 unique `(parent, role, permlevel)` keys |
...
| Effective permission regression diff | Re-run after candidate assignment sync; owner review required | `effective_permission_regression_diff_20260629T194445Z.json`; missing candidate users = 0, expected disabled users = 2, expected external-builder Item/BOM losses = 14. Remaining 11,927 losses across 26 users are dominated by Bucket A intended power-sprawl downgrades plus `hana.macinnis` undecided; do not auto-grant from this list. |
...
1. **Deploy the staged hotfix patches to production** (highest priority). Five patches are committed/staged locally but not yet on production: `v2026_06_29_finance_stock_report_access`, `v2026_06_29_transaction_role_stock_dependencies`, `v2026_06_29_operations_workspace_visibility`, `v2026_06_29_link_dependency_read_grants`, and `v2026_06_29_snapshot_stock_entry_type_permissions`. Until deployed, `patty.gomez`, `nathaniel.pantuso`, and any future `Inventory Operator` carry the live Stock Entry Type Desk-link gap. Deploy via Frappe Cloud dashboard; migrate runs all five idempotently.
...
3. **Resolve the `Quality` workspace** still restricted to retired `Builder` (owner decision on intended roles).
4. Future candidate restores should self-clean legacy `Engineering - Signoff` holders and validate both Motion and LAM external builders.
6. If any later change touches permissions, wiki fixtures, or workflow gates, rerun:
