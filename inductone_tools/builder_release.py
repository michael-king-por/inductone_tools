...
# -----------------------------
# Builder Release + As-Built Helpers
# -----------------------------
...
def generate_builder_release_bundle(build_name: str, package_name: str = None):
    ...
    co_name = getattr(build, "configuration_order", None) or getattr(build, "inductone_configuration_order", None)
    snapshot_name = getattr(build, "configured_snapshot", None) or getattr(build, "configured_bom_snapshot", None)
    ...
    if package_name:
        ...
        package_name = _resolve_bom_export_package_name(build, co)
        if package_name:
            package_doc = frappe.get_doc("BOM Export Package", package_name)

    flat_bom_file_url = _resolve_flat_bom_file_url(build, co)
    existing_bundle_url = _get_first_present(build, ["builder_release_bundle", "builder_package", "released_builder_package"])
    ...
    _sync_builder_release_document_index(
        ...
        flat_bom_file_url=flat_bom_file_url,
        replace_existing=bool(existing_bundle_url),
    )
...
def release_to_builder_now(build_name: str, package_name: str = None, note: str = None):
    ...
    - Stamps common release fields if they exist
    - Updates Configuration Order document index with the builder release bundle

    This does NOT add hard gatekeeping yet; it sets the infrastructure so that
    later validation can be layered in without reworking the release model.
    """
    ...
    _set_if_present(build, ["builder_released_on", "released_to_builder_at"], now())
    ...
    # Respect existing workflow field patterns if present, but do not assume them.
    if hasattr(build, "workflow_state"):
        try:
            build.workflow_state = "Released to Builder"
        except Exception:
            pass
    if hasattr(build, "status"):
        try:
            build.status = "Released to Builder"
        except Exception:
            pass
    ...
    return {
        ...
        "released_on": _get_first_present(build, ["builder_released_on", "released_to_builder_at"]),
    }
...
def generate_required_serial_capture_artifact(build_name: str):
    """
    Generate a CSV template listing the rows in the configured build that likely
    require serial capture. Also attempts to populate a child table when one of
    the expected fieldnames exists on InductOne Build.
    """
    build = frappe.get_doc("InductOne Build", build_name)
...
def submit_as_built_now(build_name: str, serials_json: str = None, note: str = None):
    ...
    serials_json is optional JSON array of rows. Each row may contain:
      item_code, item_name, bom_level, expected_qty, serial_no, vendor_serial_no,
      por_serial_no, parent_item_code, installed, notes

    This function intentionally avoids heavy validation for now.
    """
    build = frappe.get_doc("InductOne Build", build_name)
    rows = []
    if serials_json:
...
def close_build_from_as_built(build_name: str, note: str = None):
    ...
    This deliberately does not enforce hard validation yet.
    """
    build = frappe.get_doc("InductOne Build", build_name)
    ...
    if hasattr(build, "workflow_state"):
        try:
            build.workflow_state = "Completed"
        except Exception:
            pass
    if hasattr(build, "status"):
        try:
...
def _build_builder_release_bundle_bytes(build, configuration_order, package_doc, flat_bom_file_url, snapshot_name, top_bom):
    ...
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        # 1) Cover / manifest text
        cover_text = _builder_release_cover_text(build, configuration_order, package_doc, flat_bom_file_url, snapshot_name, top_bom, serial_rows)
        ...
        manifest["contents"].append({"path": "builder_release_manifest.json", "kind": "manifest"})

        # 2) Configuration Order PDF (best effort)
        try:
            ...
            manifest["contents"].append({"path": f"configuration_order/{configuration_order.name}.pdf", "kind": "configuration_order_pdf"})
        except Exception:
            manifest["missing"].append({"kind": "configuration_order_pdf", "reason": "print_failed", "detail": frappe.get_traceback()})

        # 3) Flat BOM CSV
        if flat_bom_file_url:
            _add_file_url_to_zip(zf, flat_bom_file_url, f"builder_bom/{os.path.basename(resolve_file_path(flat_bom_file_url) or flat_bom_file_url)}", manifest, "flat_bom")
        else:
            manifest["missing"].append({"kind": "flat_bom", "reason": "not_found"})

        # 4) BOM export package ZIP
        ...
        # 5) All document-index files currently attached to Configuration Order
        ...
        # 6) Serial capture template
        zf.writestr("serial_capture/required_serial_capture.csv", _required_serial_capture_csv_bytes(serial_rows))
        manifest["contents"].append({"path": "serial_capture/required_serial_capture.csv", "kind": "required_serial_capture"})

    # Rewrite manifest now that contents/missing are final
    # (manifest JSON inside ZIP remains the initial version; cover and return payload are still accurate enough for now)
    buf.seek(0)
...
def _add_file_url_to_zip(zf, file_url, zip_path, manifest, kind, allow_missing=False):
    abs_path = resolve_file_path(file_url)
    if not abs_path or not os.path.exists(abs_path):
        manifest["missing"].append({"kind": kind, "file_url": file_url, "zip_path": zip_path, "reason": "not_found_on_disk"})
        if allow_missing:
            return
        return
    zf.write(abs_path, zip_path)
    manifest["contents"].append({"path": zip_path, "kind": kind, "file_url": file_url})


def _resolve_bom_export_package_name(build, configuration_order):
    candidates = [
        _get_first_present(build, ["bom_export_package"]),
        _get_first_present(configuration_order, ["bom_export_package"]),
    ]
    for c in candidates:
        if c:
            return c

    # Search document index for a linked BOM export package name in notes/title.
    for row in (getattr(configuration_order, "documents", None) or []):
        ...
        if getattr(row, "source_type", None) == "BOM" and source_name:
            pass
...
def _resolve_flat_bom_file_url(build, configuration_order):
    direct = _get_first_present(build, ["flat_bom_file"]) or _get_first_present(configuration_order, ["flat_bom_file", "configured_flat_bom_file"])
    if direct:
        return direct
...
def _get_configured_rows_for_build(build_doc):
    ...
    snapshot_name = getattr(build_doc, "configured_snapshot", None) or getattr(build_doc, "configured_bom_snapshot", None)
    co_name = getattr(build_doc, "configuration_order", None) or getattr(build_doc, "inductone_configuration_order", None)
...
def _derive_required_serial_capture_rows(configured_rows):
    """
    Heuristic for now:
    - include any configured row whose Item has has_serial_no set
    - include qty / hierarchy context so the template is useful immediately
    This is intentionally simple so later validation / stricter serial requirements
    can build on the same infrastructure.
    """
    item_meta = {}
    out = []
...
def _sync_builder_release_document_index(configuration_order_name, build_name, bundle_url, package_name=None, flat_bom_file_url=None, replace_existing=False):
...
def _get_first_present(doc, fieldnames):
    for fname in fieldnames:
        try:
            if hasattr(doc, fname):
