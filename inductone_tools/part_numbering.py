...
# This batch was created after vendor/manufacturer cable part numbers such as
# 1417903 were represented as Part Number Assignment rows. Those cable numbers
# are live engineering records, and PN-ALLOC-2026-00231 is also live, but the
# batch must not redefine the go-forward POR allocator baseline. See
# docs/workflows/part-number-allocation-sequence-hardening-2026-07-15.md.
LIVE_SEQUENCE_EXCEPTION_BATCHES = {
    "PN-ALLOC-2026-00231",
}

# Sequence-authoritative POR allocations are currently in the low hundreds
# (for example 1000517 => family 1, sequence 517). Vendor/manufacturer numbers
# such as 1417903 and the accidentally continued 1417904+ / 2417917+ tranche
# have six-digit suffixes around 417xxx. They may be valid live records, but
# they are outliers for POR allocator baseline purposes.
#
# This ceiling is deliberately conservative and future-safe for the current
# allocator era: it prevents accidental manufacturer/catalog numbers from ever
# advancing the sequence while leaving normal POR allocations untouched.
SEQUENCE_AUTHORITY_MAX_REASONABLE_SUFFIX = 100000
...
def _get_current_max_sequence() -> int:
    """
    Finds the highest shared sequence suffix across sequence-authoritative
    controlled numbers.

    Sequence authority intentionally comes from the allocation engine, not from
    every numeric Item/Part Number Assignment that happens to start with 1..4.
    Vendor/manufacturer numbers such as 1417903 are valid live records but must
    not advance the POR allocator to 1417904.
    ...
    rows = frappe.get_all(
        ...
        fields=["part_number", "allocation_batch_id", "is_custom_number"],
        limit_page_length=0,
    )
    ...
    for row in rows:
        if not _is_sequence_authoritative_assignment(row):
            continue
...
def _is_sequence_authoritative_assignment(row) -> bool:
    """
    True when a Part Number Assignment is allowed to advance the allocator.

    Rules:
      - must have been generated by a Part Number Allocation Request;
      - must not be a Custom/vendor/manufacturer number;
      - must not belong to a documented live exception batch.
      - must not have an outlier sequence suffix from imported/vendor data.
    """
    allocation_batch_id = (row.get("allocation_batch_id") or "").strip()

    if not allocation_batch_id:
        return False

    if allocation_batch_id in LIVE_SEQUENCE_EXCEPTION_BATCHES:
        return False

    if cint(row.get("is_custom_number")):
        return False

    sequence = _extract_sequence(row.get("part_number"))
    if sequence is None:
        return False
